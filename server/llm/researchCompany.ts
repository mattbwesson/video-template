...
3. Respect every character cap. The copy sits in pixel-budgeted UI and a long line is clipped on screen, not wrapped. Shorter than the cap is always safe.
...
const ROUND_ONE: Batch[] = [
  ...
  {
    name: "the feed, the catch-up stories and the shout-out",
    keys: ["feed", "stories", "catchup", "composed"],
  },
];

const ROUND_TWO: Batch[] = [
  {
    name: "Spaces, the Spotlight tab and Journeys",
    keys: ["spaces", "spotlight", "journeys"],
  },
  {
    name: "Billboards, Newsletters, Chat and the HQ Agent",
    keys: ["signage", "newsletters", "chat", "hq"],
  },
  {
    name: "the article, Employee Insights and the feedback article",
    keys: ["article", "seer", "voice"],
  },
];
...
const repairOverlong = async (
  ...
  const input = [
    ...
    "",
    ...overlong.map((o) =>
      JSON.stringify({ field: o.path, max_characters: o.max, text: o.text }),
    ),
  ].join("\n");
...
export const researchCompany = async (
  ...
  const first = await collect(ROUND_ONE);
  const second = await collect(ROUND_TWO, first);
