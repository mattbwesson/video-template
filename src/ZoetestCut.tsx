 ...
 *
 * What is independent: every `<Sequence>` below, its `from`/`durationInFrames`, the order
 * the scenes play in, the transitions between them, all the shot helpers further down and
 * the several dozen frame constants they read. Retime, reorder, add or drop a scene here
 * and `L2VirginAirline` does not move.
 *
 * What is NOT: the 36 scene components this composes — `BackFromScene`,
 * `HeadquartersScene` and the rest — are the same modules `WorkvivoCut` mounts. Editing
 * one of those changes BOTH cuts. That was the deliberate trade when this copy was made:
 * forking the scenes too would have duplicated ~6,900 lines and meant fixing every
 * shared-scene bug twice. If you need a scene to differ between the two cuts, copy that
 * one scene file and point this timeline at the copy — one file, not forty.
 *
 * `REFERENCE_VIDEO` is imported rather than restated for the same reason: those are asset
 * paths, not timeline decisions, and a path fix should land once. `CUSTOMIZED_CUT_DURATION`
 * is deliberately not re-exported here — it belongs to the wizard's composition, which
 * reads it from WorkvivoCut.
 */
 ...
 * Where this cut ends.
 *
 * Read by BOTH the `Zoe-test-comms` composition in Root.tsx and the wizard's template
 * table, so the Studio and an operator's render are the same length by construction.
 *
 * The shape of the ending, since it takes two elements to read it off the tree above: the
 * customer logo wall runs 2232-2415, and the reference block mounts a SECOND `<Video>` at
 * `from={2414} trimBefore={5165}` — the film's own workvivo HQ endcard and strapline,
 * spliced on rather than left to play in sequence. So the cut ends on the endcard, not on
 * the wall and not on unrelated footage.
 *
 * That tail element asks for 134 frames, which would run to 2549. 2545 stops it four
 * frames early, which is invisible — the strapline is a static card by then — but if you
 * want the tail to play out in full, 2549 is the number.
 */
export const ZOETEST_CUT_DURATION = 2545;

export const ZoetestCut: React.FC<{
  ...
  return (
    ...
    <AbsoluteFill style={{ backgroundColor: "#000", ...theme.vars }}>
      ...
      {/* Mounted here, above the fullscreen scene and below everything else, for the same
          paint-order reason: its own iris shuts onto "the Spotlight layer mounted below it
          in the tree" (see SpacePageShot), and it overlaps Mobile Spotlight at 1630-1639,
          where it is meant to be the one on top. Same slot it has in WorkvivoCut. */}
      ...
      {/* Top Layer (888 - 1285): Workvivo Desktop Fullscreen Scene with Rotational MatchCut to Quote Card at local 192 (global 1080)
          ...
          "Top layer" is why this sits down HERE rather than up beside the other 888-1285
          material: the export paints in DOM order and ignores z-index, so a sequence has
          to be mounted late to composite above its neighbours. It overlaps two of them
          deliberately — the Desktop scene at 888-896 (8 frames) and the Livestream at
          1275-1285 (10 frames) — and in both cases it is meant to be the one on top, which
          only holds while it stays below them in this file. Same position it occupies in
          WorkvivoCut, relative to the sequences this cut still has. */}
      <Sequence
