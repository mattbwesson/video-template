...
import { PlacedSvg } from "./workvivo/symbolRegistry";
...
/** `{width: 92}` -> `{width: 92, height: 102.1}`; nothing when the width is not a plain number. */
const box = (style?: React.CSSProperties): { width: number; height: number } | undefined =>
  typeof style?.width === "number" ? { width: style.width, height: (style.width * ART_H) / ART_W } : undefined;
...
export const CursorArrow: React.FC<{
  ...
  /* The caller's placement lives on a wrapper (PlacedSvg), and the svg inside fills it.
     Putting `position:absolute; left; top` on the svg root itself is what cut the pointer
     off in the export. The box's height is derived from the caller's width so the export,
     whose layout engine does not infer it from the viewBox, still gets a real box. */
  <PlacedSvg className={className} style={style} box={box(style)}>
    {(svgStyle) => (
      <svg viewBox={`0 0 ${ART_W} ${ART_H}`} width="100%" height="100%" style={svgStyle}>
