 ...
 *
 * The mobile companion to WorkvivoSeerManagerInsights, which is the same product on
 * desktop. It is a separate component rather than a responsive mode of that one: they
 * share no layout at all, only the score tile's green ramp and the sentiment palette.
 *
 * The component is just the screen's contents — the caller owns the phone shell, exactly
 * as WorkvivoPhonesScene does with `.wm-phone` / `.wm-screen`.
 *
 * Person comes from the customization so the card reads as the tenant's own leader; the
 * reference capture's "Josh Bayliss / CEO" is Workvivo's demo data, not a fixed label.
 */

/**
 * Status-bar glyphs, prefixed sm- rather than reusing #i-signal and friends.
 *
 * Those ids belong to WorkvivoMobileHome's defs block, which only registers when that
 * component's module is loaded — importing a whole phone for three icons is a
 * side-effect dependency that breaks the moment someone tree-shakes it. Same source art,
 * own ids, same convention WorkvivoLiveReplay follows with its lr- prefix.
 */
const SpriteDefs: React.FC = () => (
  <svg
    ...
    aria-hidden
  >
    <symbol id="sm-i-signal" viewBox="0 0 17 11" fill="none">
    ...
    <symbol id="sm-i-wifi" viewBox="0 0 16 11" fill="none">
    ...
    <symbol id="sm-i-battery" viewBox="0 0 25 12" fill="none">
...
export const WorkvivoSeerManagerMobile: React.FC<WorkvivoSeerManagerMobileProps> = ({
  ...
  return (
    <div className="wsmm-screen">
      <SpriteDefs />

      <div className="wsmm-top">
      ...
      <div className="wsmm-body">
        ...
        <div className="wsmm-note">
          <div className="wsmm-note-head">
