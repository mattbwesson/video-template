 ...
 */
const radial = (cx: number, cy: number, stops: number[][], s = 1) =>
  `radial-gradient(circle at ${cx}px ${cy}px, ${stops
    .map((st) => `${rgb(st.slice(1))} ${(st[0] * s).toFixed(1)}px`)
    .join(", ")})`;
...
export const AdminCategoriesScene: React.FC = () => {
  ...
  return (
    ...
    <AbsoluteFill style={{ backgroundColor: g >= 4591 ? "#010320" : "transparent", overflow: "hidden" }}>
      ...
      {g >= 4702 && g <= 4743 && (() => {
        ...
        return (
          <>
            ...
            {g >= 4715 && g < 4743 && (
              <AbsoluteFill style={{ background: radial(TOGGLE_CX, TOGGLE_CY, BLOOM), opacity: series(4715, BLOOM_UP, g) }} />
            )}
            {/* The purple field takes the frame on 4743 with the toggle still shrinking on it. */}
            {g === 4743 && <AbsoluteFill style={{ background: radial(960, 540, FIELD) }} />}
            <div
              style={{
                position: "absolute",
                left: TRACK.x,
      ...
      {/* ── Granular Controls ───────────────────────────────────────────────── */}
      {g >= 4744 && g <= 4847 && (
        <AbsoluteFill style={{ background: radial(960, 540, FIELD) }}>
          {PLATES.map((p, i) => {
      ...
      {g >= 4840 && (() => {
        ...
        return (
          <AbsoluteFill
            ...
            <AbsoluteFill style={{ background: radial(960, 540, GLOW, glowS), opacity: glowUp }} />
